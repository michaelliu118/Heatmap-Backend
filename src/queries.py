...
SET @cols = (SELECT STRING_AGG('['+OPERATORS.OPERATOR_NAME+']', ',') FROM
(SELECT TOP 20 RV.OPERATOR_CODE
FROM
    MAIN_COMPONENT_REMOVALS RV
WHERE RV.CR_DATE BETWEEN '{0}' AND '{1}' AND 
    RV.ATA_REPORTED IS NOT NULL AND
    RV.AC_MODEL IN ('{2}', '{3}', '{4}')
GROUP BY RV.OPERATOR_CODE) A
LEFT JOIN 
MAIN_OPERATORS OPERATORS
ON OPERATORS.OPERATOR_CODE=A.OPERATOR_CODE)
...
FROM
    ...
    PIVOT( SUM(removal_rate) FOR OPERATOR_NAME IN ( '+@cols+ ')) AS PT;';
